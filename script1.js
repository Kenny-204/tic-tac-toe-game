...
for (let i = 0; i < box.length; i++) {
    box[i].addEventListener('click', function () {
        ...
        box[i].textContent = 'X';
        activePlayer = 1; // Switch to computer's turn

        // Computer's move
        let rand = Math.trunc(Math.random() * 9);
        while (box[rand].textContent != '') {
            rand = Math.trunc(Math.random() * 9);
        }
        if (box[rand].textContent == '') {
            box[rand].textContent = 'O';
            console.log(rand);
        }
        activePlayer = 0; // Switch back to player's turn        }
